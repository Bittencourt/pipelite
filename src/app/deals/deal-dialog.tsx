  ...
  const onSubmit = async (data: DealFormData) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        if (recordId) {
          const result = await updateDeal(recordId, dealData)
          if (!result.success) {
            toast.error(result.error)
            return
          }
          ...
          // `createDeal` calls `revalidatePath` before it returns, and that refresh
        ...
        if (draft) {
          let noteSaved = false
          try {
            const noteResult = await addNote("deal", recordId, draft)
          ...
          if (!noteSaved) {
            ...
            // `onRecordSaved` still runs because the record itself did land, so the board
