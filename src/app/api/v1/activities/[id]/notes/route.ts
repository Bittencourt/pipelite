...
const ENTITY_TYPE = "activity" as const
const ENTITY_LABEL = "Activity"
...
/**
 * GET /api/v1/activities/{id}/notes
 *
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  return withApiAuth(request, async (req: NextRequest) => {
    try {
      ...
      console.error("GET /api/v1/activities/[id]/notes failed:", error)
...
/**
 * POST /api/v1/activities/{id}/notes
 *
  ...
  return withApiAuth(request, async (req: NextRequest, context: ApiAuthContext) => {
    try {
      ...
      if (!result.success) {
        ...
        console.error("POST /api/v1/activities/[id]/notes failed:", result.error)
        return Problems.internalError()
      }

      return createdResponse(serializeNote(result.note))
    } catch (error) {
      console.error("POST /api/v1/activities/[id]/notes failed:", error)
      return Problems.internalError()
    }
  })
}
