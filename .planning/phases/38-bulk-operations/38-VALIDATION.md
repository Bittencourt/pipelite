...
| RSC boundary | 38-19 | 5 | BULK-01 | — | No React element crosses into a Radix `asChild` slot | unit (existing repo-wide gate) | `./node_modules/.bin/vitest run "src/app/admin/fields/[entityType]/__tests__/rsc-boundary.test.tsx"` — runs under the **DEFAULT** config, NOT `vitest.rsc.config.ts` (verified: that config's include glob is `src/**/*.rsc.test.*`, which this filename does not match; passing `--config vitest.rsc.config.ts` yields "No test files found, exiting with code 1") | ✅ exists | ⬜ pending |
