...
### Locked after plan 34-01 executed (2026-08-14)

- **D-18 (BLOCKING for every server-side call site)**: `evaluateFormula`'s resource bound is an **opt-in 4th argument**, so it is **inert unless the caller passes it**. Plan 34-01 proved the failure mode is worse than a slow request: a `while(true)` expression does not merely time out — synchronous WASM blocks the event loop, so vitest's own 10s timeout never fires and the worker had to be `pkill`ed after 300s. **One bad admin-authored formula wedges a Node worker unreclaimably.** Therefore: EVERY server-side invocation of `evaluateFormula` introduced by this phase — in `recalculateFormulas`, the cascade, the importers, and every write path — MUST pass the resource-limit options. A call site that omits the 4th argument silently reopens a denial-of-service hole (threat T-34-02). Downstream plans must assert this, not assume it.
- **D-19**: The Docker/WASM go/no-go (D-11) is **RESOLVED: YES.** `getQuickJS()` initializes in the Docker standalone build, proven from the running container (`QUICKJS_OK` via both CJS `require` and ESM `import`; the 503,134-byte `.wasm` resolves to a real `node_modules` path in `page.js.nft.json`). Measured in-container cost is **0.876 ms per evaluation**, 27% faster than the 1.195 ms host figure — so the D-13 budget of 500 evaluations is ~438 ms, comfortably inside the 2000 ms ceiling. `serverExternalPackages` was added; no Dockerfile `.wasm` copy or webpack rule was needed.
- Note two RESEARCH claims were measured wrong and are corrected in `34-01-SUMMARY.md`: `.next/server/**` *did* contain 4 `.wasm` files (the "zero" finding came from a stale March host build), and the old image built *without* `serverExternalPackages` also initialized QuickJS successfully — so assumption A1's failure mode was never actually live. The setting's real benefit is a cleaner runtime `require`, not unblocking the phase.
